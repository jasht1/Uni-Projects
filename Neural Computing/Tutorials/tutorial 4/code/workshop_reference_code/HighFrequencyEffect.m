...
Cm = 0.005;                  % Further reduce membrane capacitance (uF/cm^2)
...
I0 = 40;                     % Increase amplitude of oscillatory input (nA)
f_gamma = 80;                % Higher frequency for gamma oscillation (Hz)
noise_amplitude = 15;         % Increase noise amplitude (nA)
I_noise = noise_amplitude * randn(size(t));  % Gaussian white noise

% Generate oscillatory input (higher frequency) + noise
I_input = I0 * sin(2 * pi * f_gamma * t / 1000) + I_noise;  % Higher-frequency oscillation
