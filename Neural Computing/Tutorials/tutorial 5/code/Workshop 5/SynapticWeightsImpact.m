...
I_ext = [6 2 2];          % External current to each neuron (nA)
...
for t = 2:length(time)
    for i = 1:n_neurons
        if refractory(i) > 0
            ...
            % dV = (R_m * (I_ext(i) + I_syn)) * (dt / tau_m);
            dV = (I_ext(i) + I_syn) / tau_syn * exp(-(time(t) - last_spike_time) / tau_syn);
